...
![SELEX workflow for sample generation. Human mononucleosomal DNA was digested by MNase (1), the product was ligated with a 22-bp adapter and a 3’-overhanging thymine residue on both ends of sequence (2) then amplified and nucleosomes were <em>in vitro</em> reconstituted with recombinant histone octamers at 1:10 ratio of octamers to DNA (3). DNA was extracted from nucleosome bands in native PAGE (4) then re-amplified by PCR (5) for a total of 6 nucleosome selection rounds. The initial input and the outputs of rounds 1, 3 and 6 were sequenced [5].](https://github.com/hientrinh93/SELEX/blob/master/workflow.png)
